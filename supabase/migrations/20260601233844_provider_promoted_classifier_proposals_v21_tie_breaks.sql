...
create or replace function public.propose_fragrance_taxonomy_v21_provider_promoted(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_provider_promoted_classifier_proposal_v21',
...
as $function$
declare
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_provider_promoted_classifier_proposal_v21');
  ...
  v_classifier_model_version constant text := 'taxonomy_classifier_proposal_v21_provider_promoted_tie_breaks_2026_06_01';
...
begin
  if p_fragrance_ids is null or v_requested_count = 0 then
    raise exception 'propose_fragrance_taxonomy_v21_provider_promoted requires explicit non-empty fragrance ids';
  end if;
  ...
  if v_distinct_requested_count = 0 then
    raise exception 'propose_fragrance_taxonomy_v21_provider_promoted requires at least one non-null uuid';
  end if;

  if v_distinct_requested_count > v_max_ids then
    raise exception 'propose_fragrance_taxonomy_v21_provider_promoted accepts at most % fragrance ids per call', v_max_ids;
  end if;
        ...
        family_scores as (
          select
            ...
            sum(fm.matched_weight)
              ...
              + case when fm.candidate_family = 'amber-oriental'
                        and exists (
                          select 1
                          from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
                          where public.taxonomy_norm_text_v1(token) like '%saffron%'
                        )
                        and exists (
                          select 1
                          from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
                          where public.taxonomy_norm_text_v1(token) like '%rose%'
                             or public.taxonomy_norm_text_v1(token) like '%amber%'
                             or public.taxonomy_norm_text_v1(token) like '%oud%'
                             or public.taxonomy_norm_text_v1(token) like '%agarwood%'
                             or public.taxonomy_norm_text_v1(token) like '%olibanum%'
                             or public.taxonomy_norm_text_v1(token) like '%resin%'
                        )
                     then 3 else 0 end
              + case when fm.candidate_family = 'leather'
                        and coalesce(v_row.sweet_base_persistence_likelihood, false)
                        and coalesce(v_row.drydown_anchor_strength, '') in ('MODERATE', 'HIGH')
                        and exists (
                          select 1
                          from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
                          where public.taxonomy_norm_text_v1(token) like '%leather%'
                             or public.taxonomy_norm_text_v1(token) like '%suede%'
                             or public.taxonomy_norm_text_v1(token) like '%birch%'
                             or public.taxonomy_norm_text_v1(token) like '%animalic%'
                        )
                     then 2 else 0 end
              ...
              - case when fm.candidate_family = 'citrus-cologne'
                        ...
                        and exists (
                          ...
                          where public.taxonomy_norm_text_v1(token) like '%leather%'
                             or public.taxonomy_norm_text_v1(token) like '%suede%'
                             or public.taxonomy_norm_text_v1(token) like '%amber%'
                             or public.taxonomy_norm_text_v1(token) like '%vanilla%'
                             or public.taxonomy_norm_text_v1(token) like '%gourmand%'
                        )
                     then 5 else 0 end
      ...
      if coalesce(nullif(v_proposed_universal_family_key, ''), '') = '' then
            ...
            order by et.facet_key, xt.source_kind, xt.raw_token, et.term, matched_weight desc
	          ...
	          scored_base as (
	            select
	              ...
	              sum(matched_weight) as base_score
	            from matched_terms
	          ...
	          scored as (
	            select
	              ...
	              greatest(
	                ...
	                base_score
	                + case
	                    when facet_key = 'spicy'
	                     and exists (
	                       select 1
	                       from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
	                       where public.taxonomy_norm_text_v1(token) like '%warm spicy%'
	                          or public.taxonomy_norm_text_v1(token) like '%fresh spicy%'
	                          or public.taxonomy_norm_text_v1(token) like '%saffron%'
	                          or public.taxonomy_norm_text_v1(token) like '%cardamom%'
	                          or public.taxonomy_norm_text_v1(token) like '%nutmeg%'
	                          or public.taxonomy_norm_text_v1(token) like '%pepper%'
	                          or public.taxonomy_norm_text_v1(token) like '%cinnamon%'
	                     )
	                    then 4 else 0
	                  end
	                + case
	                    when facet_key = 'leather'
	                     and exists (
	                       select 1
	                       from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
	                       where public.taxonomy_norm_text_v1(token) like '%leather%'
	                ...
	                + case
	                    when facet_key = 'resin'
	                     and exists (
	                       select 1
	                       from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
	                       where public.taxonomy_norm_text_v1(token) like '%resin%'
	                          or public.taxonomy_norm_text_v1(token) like '%olibanum%'
	                          or public.taxonomy_norm_text_v1(token) like '%benzoin%'
	                          or public.taxonomy_norm_text_v1(token) like '%labdanum%'
	                          or public.taxonomy_norm_text_v1(token) like '%styrax%'
	                     )
	                    then 2 else 0
	                  end
	                - case
	                    when facet_key = 'balsamic'
	                     and not exists (
	                       select 1
	                       from unnest(coalesce(v_row.notes, array[]::text[]) || coalesce(v_row.accords, array[]::text[])) token
	                       where public.taxonomy_norm_text_v1(token) like '%resin%'
	                          ...
	                          or public.taxonomy_norm_text_v1(token) like '%labdanum%'
	                          or public.taxonomy_norm_text_v1(token) like '%styrax%'
	                          or public.taxonomy_norm_text_v1(token) like '%myrrh%'
	                          or public.taxonomy_norm_text_v1(token) like '%incense%'
	                     )
	                    then 2 else 0
	                  end
	                - case
	                    when facet_key = 'woody'
	                     and coalesce(v_proposed_universal_family_key, '') <> 'woody'
	                     and coalesce(v_row.woody_amber_dominance_likelihood, '') = 'LOW'
	                     and (
	                       coalesce(note_hits, 0) < 3
	                       or coalesce(array_length(matched_terms, 1), 0) < 3
	                     )
	                    then 5 else 0
	                  end
	              ) as score,
        ...
        if exists (
          ...
          if v_proposed_universal_family_key in ('citrus-cologne', 'fresh-aquatic')
            ...
            v_role_rationale := array['fresh_family_selected_from_evidence', 'lighter_projection_profile', 'provider_promoted_v21'];
            ...
            v_role_rationale := array['sweet_diffusive_profile', 'soft_rounding_performance', 'provider_promoted_v21'];
          elsif (
            ...
            v_primary_role_confidence := 0.67;
            v_secondary_role_confidence := 0.61;
            v_role_rationale := array['heavier_structural_family', 'anchor_weight_supported_by_performance', 'provider_promoted_v21'];
            ...
            v_role_rationale := array['powdery_or_musky_texture', 'low_anchor_profile', 'provider_promoted_v21'];
            ...
            v_role_rationale := array['woody_structure_supported', 'performance_confirms_anchor_use_case', 'provider_promoted_v21'];
            ...
            v_primary_role_confidence := 0.63;
            v_secondary_role_confidence := 0.58;
            v_role_rationale := array['fresh_facet_cluster', 'lighter_projection_profile', 'provider_promoted_v21'];
          elsif v_proposed_universal_family_key = 'gourmand'
            ...
            v_role_rationale := array['sweeter_statement_profile', 'heavier_projection_than_softener_lane', 'provider_promoted_v21'];
          elsif coalesce(v_row.drydown_anchor_strength, '') = 'LOW' then
            v_primary_role := 'bridge';
            v_secondary_role := null;
            v_primary_role_confidence := 0.54;
            v_secondary_role_confidence := null;
            v_role_rationale := array['fallback_low_anchor_role', 'provider_promoted_v21'];
            v_has_manual_review_penalty := true;
          else
            v_primary_role := 'accent';
            v_secondary_role := null;
            ...
            v_secondary_role_confidence := null;
            v_role_rationale := array['fallback_statement_role', 'provider_promoted_v21'];
            v_has_manual_review_penalty := true;
          end if;
          ...
          if exists (
            ...
            if v_facet_count < 3 then
              ...
              v_proposal_status := 'needs_manual_review';
              v_blocker_reason := 'low_provider_promoted_v21_confidence';
            end if;
            ...
            v_evidence_summary := jsonb_build_object(
              'proposal_basis', 'provider_promoted_notes_only_queue_routed_v21_tie_breaks',
            ...
            v_source_summary := jsonb_build_object(
              ...
              'basis', 'provider_promoted_enrichment_notes_only_v21_tie_breaks',
...
revoke all on function public.propose_fragrance_taxonomy_v21_provider_promoted(uuid[], text, boolean) from public;
revoke all on function public.propose_fragrance_taxonomy_v21_provider_promoted(uuid[], text, boolean) from anon;
revoke all on function public.propose_fragrance_taxonomy_v21_provider_promoted(uuid[], text, boolean) from authenticated;
grant execute on function public.propose_fragrance_taxonomy_v21_provider_promoted(uuid[], text, boolean) to service_role;

comment on function public.propose_fragrance_taxonomy_v21_provider_promoted(uuid[], text, boolean) is
