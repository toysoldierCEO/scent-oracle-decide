...
create or replace function public.accept_fragrance_taxonomy_proposals_v1_provider_promoted(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_provider_promoted_proposal_acceptance_v1',
  p_dry_run boolean default true,
  p_model_version text default 'taxonomy_classifier_proposal_v1_provider_promoted_2026_05_28'
)
returns jsonb
language plpgsql
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_provider_promoted_proposal_acceptance_v1');
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'accept_fragrance_taxonomy_proposals_v1_provider_promoted requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'accept_fragrance_taxonomy_proposals_v1_provider_promoted accepts at most % fragrance ids per call', v_max_ids;
  end if;

  if p_model_version is null or btrim(p_model_version) = '' then
    raise exception 'accept_fragrance_taxonomy_proposals_v1_provider_promoted requires a non-empty model version';
  end if;
  ...
  loop
    ...
    if v_row.fragrance_id is null then
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif coalesce(v_row.evidence_summary ->> 'proposal_basis', '') <> 'provider_promoted_notes_only_queue_routed' then
      v_result_status := 'rejected';
        ...
        if p_dry_run then
          ...
          with inserted_facets as (
            ...
            select
              ...
              'taxonomy_proposal_acceptance_v1_provider_promoted',
              jsonb_build_object(
                ...
                'provider', v_row.provider,
                'enrichment_status', v_row.enrichment_status,
                'evidence_basis', 'provider_promoted_notes_only_queue_routed',
  ...
  return jsonb_build_object(
    'ok', true,
    'function_version', 'provider_promoted_proposal_acceptance_v1_2026_05_29',
    'dry_run', p_dry_run,
    'actor_label', v_actor_label,
...
comment on function public.accept_fragrance_taxonomy_proposals_v1_provider_promoted(uuid[], text, boolean, text) is
  'Accepts explicit provider-promoted Fragella taxonomy proposals into final taxonomy surfaces for the safe classifier lane without mutating fragrance truth or refreshing queue/performance.';
