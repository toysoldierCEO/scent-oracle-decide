create or replace function public.get_signed_in_recent_layer_rotation_exclusions_v2(
  p_user_id uuid,
...
declare
  ...
  v_pair_adjacent_day uuid[] := '{}'::uuid[];
  v_pair_recent_window uuid[] := '{}'::uuid[];
  v_pair_weekly_window uuid[] := '{}'::uuid[];
...
begin
  ...
  loop
    ...
    select
      coalesce(
        nullif(dm.state_json->'lockedCard'->>'fragrance_id', ''),
        nullif(dm.state_json->'manualHeroCard'->>'fragrance_id', ''),
        nullif(dm.state_json->'lockedResolvedCurrentCard'->>'fragrance_id', '')
      )::uuid as history_hero_id,
      coalesce(
        nullif(dm.state_json->'lockedLayerCard'->>'fragrance_id', ''),
        nullif(dm.state_json->'manualLayerCard'->>'fragrance_id', ''),
        nullif(dm.state_json->'lockedResolvedCurrentCard'->'layer'->>'id', ''),
        nullif(dm.state_json->'lockedResolvedCurrentCard'->'layer'->>'fragrance_id', '')
      )::uuid as history_layer_id
    into v_history_hero_id, v_history_layer_id
    from public.odara_signed_in_day_memory dm
    where dm.user_id = p_user_id
      and dm.date_key = v_history_date
      and lower(
        coalesce(
          nullif(dm.context_key, ''),
          nullif(dm.state_json->>'lockedContext', ''),
          v_effective_context
        )
      ) = v_effective_context
      and (
        coalesce(dm.state_json->>'lockState', '') = 'locked'
        or dm.state_json->'lockedLayerCard' is not null
        or dm.state_json->'manualLayerCard' is not null
        or dm.state_json->'lockedResolvedCurrentCard'->'layer' is not null
      )
    order by dm.updated_at desc nulls last
    limit 1;

    if v_history_layer_id is null then
      select *
      ...
      if v_history_hero_id is not null then
    ...
    if p_anchor_fragrance_id is not null
       and v_history_hero_id = p_anchor_fragrance_id then
      if v_offset = 1
         and not (v_history_layer_id = any(coalesce(v_pair_adjacent_day, '{}'::uuid[]))) then
        v_pair_adjacent_day := array_append(v_pair_adjacent_day, v_history_layer_id);
      end if;

      if v_offset <= least(v_effective_lookback, 7)
         and not (v_history_layer_id = any(coalesce(v_pair_recent_window, '{}'::uuid[]))) then
        v_pair_recent_window := array_append(v_pair_recent_window, v_history_layer_id);
      end if;

      if not (v_history_layer_id = any(coalesce(v_pair_weekly_window, '{}'::uuid[]))) then
        v_pair_weekly_window := array_append(v_pair_weekly_window, v_history_layer_id);
      end if;
    end if;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'pair_adjacent_day', coalesce(to_jsonb(v_pair_adjacent_day), '[]'::jsonb),
    'pair_recent_window', coalesce(to_jsonb(v_pair_recent_window), '[]'::jsonb),
    'pair_weekly_window', coalesce(to_jsonb(v_pair_weekly_window), '[]'::jsonb),
...
begin
  ...
  return jsonb_build_object(
    ...
    'overlap_policy', to_jsonb('hard_pair_and_adjacent_soft_recent_soft_weekly_fallback'::text),
    'modes', v_modes
  );
end;
$function$;
