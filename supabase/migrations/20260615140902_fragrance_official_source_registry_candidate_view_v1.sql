...
comment on view public.fragrance_official_source_registry_candidate_view_v1 is
  'Read-only registry review view for official source evidence. It centralizes next_action_bucket, recapture_status, batch_1c_capture_candidate, and patch_safe classification over fragrance_official_source_evidence_registry_v1 without mutating public.fragrances, calling helper functions, refreshing queue/performance, or inserting audit rows.';

comment on column public.fragrance_official_source_registry_candidate_view_v1.next_action_bucket is
  'Primary evidence-use classification for review and downstream helper planning.';
...
comment on column public.fragrance_official_source_registry_candidate_view_v1.patch_safe is
  'True only when underlying registry evidence is helper-compatible, ready_for_dry_run, and free of duplicate, ambiguity, rejection, or stale-state blockers.';
