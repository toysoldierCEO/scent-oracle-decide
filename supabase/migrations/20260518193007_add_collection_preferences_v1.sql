create table if not exists public.user_fragrance_preferences_v1 (
  user_id uuid not null references auth.users(id) on delete cascade,
  fragrance_id uuid not null references public.fragrances(id) on delete cascade,
  preference_state text not null check (preference_state in ('liked', 'loved')),
...
create index if not exists idx_user_fragrance_preferences_v1_user_state
  on public.user_fragrance_preferences_v1 (user_id, preference_state, updated_at desc);

create index if not exists idx_user_fragrance_preferences_v1_fragrance
  on public.user_fragrance_preferences_v1 (fragrance_id);

alter table public.user_fragrance_preferences_v1 enable row level security;

drop policy if exists user_fragrance_preferences_v1_select_self on public.user_fragrance_preferences_v1;
create policy user_fragrance_preferences_v1_select_self
on public.user_fragrance_preferences_v1
for select
to authenticated
using (auth.uid() = user_id);

drop policy if exists user_fragrance_preferences_v1_insert_self on public.user_fragrance_preferences_v1;
create policy user_fragrance_preferences_v1_insert_self
on public.user_fragrance_preferences_v1
for insert
to authenticated
with check (auth.uid() = user_id);

drop policy if exists user_fragrance_preferences_v1_update_self on public.user_fragrance_preferences_v1;
create policy user_fragrance_preferences_v1_update_self
on public.user_fragrance_preferences_v1
for update
to authenticated
using (auth.uid() = user_id)
...
drop policy if exists user_fragrance_preferences_v1_delete_self on public.user_fragrance_preferences_v1;
create policy user_fragrance_preferences_v1_delete_self
on public.user_fragrance_preferences_v1
for delete
to authenticated
using (auth.uid() = user_id);

revoke all on table public.user_fragrance_preferences_v1 from public, anon, authenticated;
grant select, insert, update, delete on public.user_fragrance_preferences_v1 to service_role;
...
begin
  ...
  if v_next_state not in ('liked', 'loved', 'neutral') then
    raise exception 'p_next_state must be liked, loved, or neutral.';
  end if;
      ...
      'items', coalesce(
        (
          select jsonb_agg(
            ...
            order by
              case i.preference_state
                when 'loved' then 0
                when 'liked' then 1
                else 2
              end,
              case i.collection_status
                when 'signature' then 0
                else 1
              end,
              lower(coalesce(i.brand, '')),
    ...
    select jsonb_build_object(
      ...
      'items', coalesce(
        (
          select jsonb_agg(
            jsonb_build_object(
              'fragrance_id', gb.fragrance_id,
              'name', gb.name,
              'brand', gb.brand,
              'family_key', gb.family_key,
              'family_label', gb.family_label,
              'image_url', ia.image_url,
              'thumbnail_url', ia.thumbnail_url,
              'collection_status', 'guest_demo',
      ...
      select jsonb_build_object(
        ...
        'insights', jsonb_build_object(
          ...
          'signature_gravity', jsonb_build_object(
            ...
            'source', case
              when pr.preference_count > 0 then 'user_fragrance_preferences_v1 + user_collection_effective_items_v2 + wear_events'
              when cs.bottle_count > 0 then 'user_collection_effective_items_v2 + wear_events'
              else null
            end,
            'empty_reason', case
              when pr.preference_count > 0 or cs.bottle_count > 0 then null
        ...
        'data_quality', jsonb_build_object(
          ...
          'has_preferences', (pr.preference_count > 0),
