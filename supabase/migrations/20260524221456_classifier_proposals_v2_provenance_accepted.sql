...
create or replace function public.propose_fragrance_taxonomy_v2(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_classifier_proposal_v2_provenance',
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_classifier_proposal_v2_provenance');
  ...
  v_classifier_model_version constant text := 'taxonomy_classifier_proposal_v2_provenance_accepted_2026_05_24';
  v_source_model_version constant text := 'provenance_review_v1_operational_memory_2026_05_24';
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'propose_fragrance_taxonomy_v2 requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'propose_fragrance_taxonomy_v2 accepts at most % fragrance ids per call', v_max_ids;
  end if;
  ...
  for v_row in
    ...
    select
      ...
      ex.proposal_id as existing_v2_proposal_id
    from requested req
  ...
  loop
    ...
    if v_row.fragrance_id is null then
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.existing_v2_proposal_id is not null then
      ...
      from (
        with raw_tokens as (
          select unnest(coalesce(v_row.notes, array[]::text[])) as token, 'payload'::text as source_kind
          union all
          select unnest(coalesce(v_row.accords, array[]::text[])) as token, 'payload'::text as source_kind
          union all
        ...
        matched_terms as (
          select
            ...
            tt.source_kind,
            case
              when tt.source_kind = 'family' then et.evidence_weight + 1
      ...
      if coalesce(v_facet_keys, array[]::text[]) && array['citrus', 'green', 'marine', 'ozonic']::text[]
        ...
        v_role_rationale := array['fresh_or_airy_facet_cluster', 'lighter_projection_profile'];
      ...
      v_source_summary := jsonb_build_object(
        ...
        'basis', 'provenance_accepted_controlled_classifier_review',
        'provenance_review_id', v_row.provenance_review_id,
        'provenance_review_status', v_row.provenance_review_status,
        'provenance_category', v_row.provenance_category,
        ...
        'source_snapshot', coalesce(v_row.provenance_source_snapshot, '{}'::jsonb),
...
comment on function public.propose_fragrance_taxonomy_v2(uuid[], text, boolean) is
  'Controlled classifier proposal v2 for provenance-accepted rows only. Requires explicit fragrance ids, reads the fast current queue plus latest provenance review memory, writes proposal rows only to fragrance_taxonomy_proposals_v1, and never writes final taxonomy or mutates public.fragrances.';

revoke all on function public.propose_fragrance_taxonomy_v2(uuid[], text, boolean) from public, anon, authenticated;
grant execute on function public.propose_fragrance_taxonomy_v2(uuid[], text, boolean) to service_role;
