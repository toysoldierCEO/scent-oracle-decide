...
create or replace function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_provider_promoted_proposal_acceptance_v21',
  p_dry_run boolean default true,
  p_model_version text default 'taxonomy_classifier_proposal_v21_provider_promoted_tie_breaks_2026_06_01'
)
returns jsonb
language plpgsql
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_provider_promoted_proposal_acceptance_v21');
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'accept_fragrance_taxonomy_proposals_v21_provider_promoted requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'accept_fragrance_taxonomy_proposals_v21_provider_promoted accepts at most % fragrance ids per call', v_max_ids;
  end if;

  if p_model_version is null or btrim(p_model_version) = '' then
    raise exception 'accept_fragrance_taxonomy_proposals_v21_provider_promoted requires a non-empty model version';
  end if;

  if p_model_version <> 'taxonomy_classifier_proposal_v21_provider_promoted_tie_breaks_2026_06_01' then
    raise exception 'accept_fragrance_taxonomy_proposals_v21_provider_promoted only accepts model version taxonomy_classifier_proposal_v21_provider_promoted_tie_breaks_2026_06_01';
  end if;
  ...
  loop
    ...
    if v_row.fragrance_id is null then
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif coalesce(v_row.evidence_summary ->> 'proposal_basis', '') <> 'provider_promoted_notes_only_queue_routed_v21_tie_breaks' then
      v_result_status := 'rejected';
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif coalesce(v_row.source_summary ->> 'basis', '') <> 'provider_promoted_enrichment_notes_only_v21_tie_breaks' then
      v_result_status := 'rejected';
        ...
        if p_dry_run then
          ...
          with inserted_facets as (
            ...
            select
              ...
              'taxonomy_proposal_acceptance_v21_provider_promoted',
              jsonb_build_object(
                ...
                'provider', v_row.provider,
                'enrichment_status', v_row.enrichment_status,
                'evidence_basis', 'provider_promoted_notes_only_queue_routed_v21_tie_breaks',
  ...
  return jsonb_build_object(
    'ok', true,
    'function_version', 'provider_promoted_proposal_acceptance_v21_2026_06_01',
    'dry_run', p_dry_run,
    'actor_label', v_actor_label,
...
comment on function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(uuid[], text, boolean, text) is
  'Accepts explicit v2.1 tie-break provider-promoted Fragella taxonomy proposals into final taxonomy surfaces for the safe classifier lane without mutating fragrance truth or refreshing queue/performance.';

revoke all on function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(uuid[], text, boolean, text) from public;
revoke all on function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(uuid[], text, boolean, text) from anon;
revoke all on function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(uuid[], text, boolean, text) from authenticated;
grant execute on function public.accept_fragrance_taxonomy_proposals_v21_provider_promoted(uuid[], text, boolean, text) to service_role;
