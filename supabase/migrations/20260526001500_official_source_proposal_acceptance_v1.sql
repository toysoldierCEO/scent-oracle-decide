...
create or replace function public.accept_fragrance_taxonomy_proposals_v4_official_source_queue(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_official_source_proposal_acceptance_v1',
  p_dry_run boolean default true,
  p_model_version text default 'taxonomy_classifier_proposal_v4_official_source_queue_2026_05_25'
)
returns jsonb
language plpgsql
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_official_source_proposal_acceptance_v1');
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'accept_fragrance_taxonomy_proposals_v4_official_source_queue requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'accept_fragrance_taxonomy_proposals_v4_official_source_queue accepts at most % fragrance ids per call', v_max_ids;
  end if;

  if p_model_version is null or btrim(p_model_version) = '' then
    raise exception 'accept_fragrance_taxonomy_proposals_v4_official_source_queue requires a non-empty model version';
  end if;
  ...
  for v_row in
    ...
    select
      ...
      e.performance_refresh_model_version,
      ...
      e.official_source_payload,
      e.official_source_verification_summary,
  ...
  loop
    ...
    v_proposal_snapshot := jsonb_build_object(
      ...
      'official_source_confidence_numeric', v_row.source_confidence_numeric,
      'official_source_type', v_row.official_source_type,
      ...
      'official_source_duplicate_audit_count', coalesce(v_row.duplicate_audit_count, 0),
      'official_source_evidence_summary', coalesce(v_row.official_source_evidence_summary, '{}'::jsonb),
      'official_source_verification_summary', coalesce(v_row.official_source_verification_summary, '{}'::jsonb),
      'performance_refresh_run_id', v_row.performance_refresh_run_id,
      'performance_refresh_status', v_row.performance_refresh_status,
      'performance_refresh_model_version', v_row.performance_refresh_model_version,
      ...
      'provenance_performance_snapshot', coalesce(v_row.provenance_performance_snapshot, '{}'::jsonb),
      'provenance_taxonomy_snapshot', coalesce(v_row.provenance_taxonomy_snapshot, '{}'::jsonb),
    ...
    if v_row.fragrance_id is null then
      ...
      v_result_status := 'rejected';
      v_blocker_reason := 'queue_not_official_source_classifier_review';
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.queue_recommended_next_action is not null
       and v_row.queue_recommended_next_action <> 'controlled_classifier_review_candidate' then
      v_result_status := 'rejected';
      v_blocker_reason := 'queue_not_controlled_classifier_review_candidate';
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.selected_official_source_backfill_audit_id is null then
      v_result_status := 'rejected';
      v_blocker_reason := 'official_source_backfill_missing';
      v_rejected_count := v_rejected_count + 1;
    elsif coalesce(v_row.has_matching_official_source_backfill, false) is false then
      v_result_status := 'rejected';
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif coalesce(v_row.has_official_note_pyramid, false) is false then
      v_result_status := 'rejected';
      v_blocker_reason := 'official_source_note_pyramid_missing';
      v_rejected_count := v_rejected_count + 1;
        ...
        if p_dry_run then
          ...
          with inserted_facets as (
            ...
            select
              ...
              'taxonomy_proposal_acceptance_v4_official_source_queue',
              jsonb_build_object(
  ...
  return jsonb_build_object(
    'ok', true,
    'function_version', 'official_source_proposal_acceptance_gate_v1_2026_05_26',
    'dry_run', p_dry_run,
    'actor_label', v_actor_label,
...
comment on function public.accept_fragrance_taxonomy_proposals_v4_official_source_queue(uuid[], text, boolean, text) is
  'Explicit-ID official-source-aware acceptance gate for classifier proposal v4 rows. Dry-run previews write nothing. Live acceptance writes final facets, wardrobe roles, taxonomy review, proposal status, and acceptance audit only; it never mutates public.fragrances or refreshes performance.';

revoke all on function public.accept_fragrance_taxonomy_proposals_v4_official_source_queue(uuid[], text, boolean, text) from public, anon, authenticated;
grant execute on function public.accept_fragrance_taxonomy_proposals_v4_official_source_queue(uuid[], text, boolean, text) to service_role;
