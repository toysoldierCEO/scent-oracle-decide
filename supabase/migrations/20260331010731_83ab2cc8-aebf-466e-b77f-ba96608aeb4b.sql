...
drop function if exists public.get_todays_oracle_v3(uuid, numeric, text, text);
...
declare
  ...
  v_reason text;
begin
  -- MAIN PICK (deterministic by user + context + date)
  select f.*
  into v_main_row
  ...
  -- LAYER (must be different family than main)
  select f.*
  into v_layer_row
  from public.fragrances f
  ...
  return jsonb_build_object(
    ...
    'layer',
      case when v_layer_row is not null then jsonb_build_object(
