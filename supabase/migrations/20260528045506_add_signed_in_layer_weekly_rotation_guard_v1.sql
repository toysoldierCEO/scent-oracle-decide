...
begin
  ...
  loop
    ...
    if not (v_history_layer_id = any(coalesce(v_recent_window, '{}'::uuid[]))) then
...
begin
  ...
  select coalesce(array_agg(value::uuid), '{}'::uuid[])
  into v_recent_preview_soft_exclude
  ...
  return jsonb_build_object(
    ...
    'overlap_policy', to_jsonb('hard_primary_soft_preview_fallback'::text),
    'modes', v_modes
  );
end;
$function$;
