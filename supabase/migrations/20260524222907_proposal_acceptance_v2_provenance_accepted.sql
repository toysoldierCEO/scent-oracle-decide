...
create or replace function public.accept_fragrance_taxonomy_proposals_v2(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_proposal_acceptance_v2_provenance',
  p_dry_run boolean default true,
  p_model_version text default 'taxonomy_classifier_proposal_v2_provenance_accepted_2026_05_24'
)
returns jsonb
language plpgsql
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_proposal_acceptance_v2_provenance');
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'accept_fragrance_taxonomy_proposals_v2 requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'accept_fragrance_taxonomy_proposals_v2 accepts at most % fragrance ids per call', v_max_ids;
  end if;

  if p_model_version is null or btrim(p_model_version) = '' then
    raise exception 'accept_fragrance_taxonomy_proposals_v2 requires a non-empty model version';
  end if;
  ...
  loop
    ...
    if v_row.fragrance_id is null then
      ...
      v_result_status := 'rejected';
      v_blocker_reason := 'latest_provenance_not_accepted';
      v_rejected_count := v_rejected_count + 1;
      ...
      v_result_status := 'rejected';
      v_blocker_reason := 'queue_not_controlled_classifier_review';
      v_rejected_count := v_rejected_count + 1;
        ...
        if p_dry_run then
          ...
          with inserted_facets as (
            ...
            select
              ...
              'taxonomy_proposal_acceptance_v2',
              jsonb_build_object(
  ...
  return jsonb_build_object(
    'ok', true,
    'function_version', 'taxonomy_proposal_acceptance_gate_v2_provenance_2026_05_24',
    'dry_run', p_dry_run,
    'actor_label', v_actor_label,
...
comment on function public.accept_fragrance_taxonomy_proposals_v2(uuid[], text, boolean, text) is
  'Explicit-ID provenance-aware acceptance gate for classifier proposal v2 rows. Dry-run previews write nothing. Live acceptance writes final facets, wardrobe roles, taxonomy review, proposal status, and acceptance audit only; it never mutates public.fragrances or refreshes performance.';

revoke all on function public.accept_fragrance_taxonomy_proposals_v2(uuid[], text, boolean, text) from public, anon, authenticated;
grant execute on function public.accept_fragrance_taxonomy_proposals_v2(uuid[], text, boolean, text) to service_role;
