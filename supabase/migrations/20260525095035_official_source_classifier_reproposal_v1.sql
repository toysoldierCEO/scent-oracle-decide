create or replace function public.propose_fragrance_taxonomy_v3_official_source_backfill(
  p_fragrance_ids uuid[],
  p_actor_label text default 'codex_classifier_proposal_v3_official_source',
...
declare
  v_distinct_ids uuid[];
  v_actor_label text := coalesce(nullif(btrim(p_actor_label), ''), 'codex_classifier_proposal_v3_official_source');
  ...
  v_classifier_model_version constant text := 'taxonomy_classifier_proposal_v3_official_source_backfill_2026_05_25';
...
begin
  ...
  if v_requested_count = 0 then
    raise exception 'propose_fragrance_taxonomy_v3_official_source_backfill requires explicit non-empty fragrance ids';
  end if;

  if v_requested_count > v_max_ids then
    raise exception 'propose_fragrance_taxonomy_v3_official_source_backfill accepts at most % fragrance ids per call', v_max_ids;
  end if;
  ...
  for v_row in
    ...
    latest_backfill as (
      select distinct on (fragrance_id)
        fragrance_id,
        id as backfill_audit_id,
        source_type,
        source_url as audit_source_url,
        source_confidence as audit_source_confidence,
        source_payload,
        source_verification_summary,
        changed_fields,
        created_at as backfill_created_at
      from public.fragrance_source_backfill_audit_v1
      where fragrance_id = any (v_distinct_ids)
        and source_type = 'official_brand'
      order by fragrance_id, created_at desc, id desc
    )
    select
      ...
      ex.proposal_id as existing_v3_proposal_id,
      ...
      pp.prior_created_by,
      pp.prior_created_at,
      lb.backfill_audit_id,
      lb.audit_source_url,
      lb.audit_source_confidence,
      lb.source_payload as backfill_source_payload,
      lb.source_verification_summary,
      lb.changed_fields as backfill_changed_fields,
      lb.backfill_created_at
    from requested req
  ...
  loop
    ...
    if v_row.fragrance_id is null then
    ...
    elsif v_row.queue_state <> 'provenance_accepted_pending_classifier_review'
       or v_row.queue_lane <> 'controlled_classifier_review' then
      v_result_status := 'rejected';
      v_blocker_reason := 'not_controlled_classifier_review_candidate';
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.provenance_review_status <> 'provenance_accepted' then
      v_result_status := 'rejected';
      v_blocker_reason := 'provenance_status_not_accepted';
      v_rejected_count := v_rejected_count + 1;
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.audit_source_url is distinct from v_row.source_url
      ...
      v_rejected_count := v_rejected_count + 1;
    elsif v_row.existing_v3_proposal_id is not null then
      ...
      from (
        ...
        scored as (
          select
            ...
            case
              when sum(matched_weight) >= 8 then 0.81
              when sum(matched_weight) >= 6 then 0.76
              when sum(matched_weight) >= 4 then 0.71
              else 0.66
      ...
      if coalesce(v_facet_keys, array[]::text[]) && array['citrus', 'green', 'marine', 'ozonic']::text[]
        ...
        v_role_rationale := array['high_projection_or_spray_caution', 'structured_core_use_case'];
        ...
        v_primary_role_confidence := 0.77;
        v_secondary_role_confidence := 0.69;
        v_role_rationale := array['dense_gourmand_profile', 'higher_signal_statement_candidate'];
        ...
        v_role_rationale := array['leather_structural_core', 'contrast_support_role'];
      elsif (
        ...
        v_secondary_role_confidence := 0.66;
        v_role_rationale := array['sweet_color_role', 'connective_wear_flexibility'];
        ...
        v_role_rationale := array['diffusive_texture_support', 'soft_halo_profile'];
      else
        v_primary_role := 'anchor';
        v_secondary_role := null;
      ...
      v_proposed_confidence := least(
        ...
        + case
            when v_row.provenance_review_status = 'provenance_accepted' then 0.04
            else 0
          end
        + case
        ...
        + case
            when v_row.backfill_audit_id is not null
             and coalesce(nullif(v_row.source_url, ''), '') <> ''
             and coalesce(nullif(v_row.source_confidence, ''), '') in ('0.95', '0.99', '1', '1.0', 'high')
            then 0.04
            else 0
          end
        - case
...
revoke all on function public.propose_fragrance_taxonomy_v3_official_source_backfill(uuid[], text, boolean) from public;
revoke all on function public.propose_fragrance_taxonomy_v3_official_source_backfill(uuid[], text, boolean) from anon;
revoke all on function public.propose_fragrance_taxonomy_v3_official_source_backfill(uuid[], text, boolean) from authenticated;
grant execute on function public.propose_fragrance_taxonomy_v3_official_source_backfill(uuid[], text, boolean) to service_role;

comment on function public.propose_fragrance_taxonomy_v3_official_source_backfill(uuid[], text, boolean) is
  'Generates proposal-only classifier rows for explicit official-source-backed fragrances using a new model version, preserving older proposal lineage and never writing final taxonomy.';
