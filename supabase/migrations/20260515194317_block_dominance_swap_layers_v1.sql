...
begin
  ...
  select
    ...
    case
      when v_mode = 'wild' then '1:1'
      else '2:1'
    end as ratio_hint,

    case
      when c.support_role_estimate = 'lingering-base' then 'Depth-support application'
      when c.support_role_estimate = 'lift' then 'Lift-support application'
      when c.support_role_estimate in ('support', 'driver') and v_mode = 'smooth' then 'Soft-overlap application'
      when c.support_role_estimate in ('support', 'driver') and v_mode = 'balance' then 'Support-first application'
      when v_mode = 'bold' then 'Contrast-support application'
      when v_mode = 'wild' then 'Experimental contrast application'
      else 'Balanced application'
    end as application_style,

    case
      when v_mode = 'wild' then
        c.anchor_name || ': 1 spray chest | ' || c.layer_name || ': 1 spray opposite side of neck or wrist'
      when c.support_role_estimate = 'lift' then
        c.anchor_name || ': 1 spray chest, 1 spray neck | ' || c.layer_name || ': 1 spray wrist or collarbone'
      when c.support_role_estimate = 'lingering-base' then
        c.anchor_name || ': 1 spray chest, 1 spray back of neck | ' || c.layer_name || ': 1 spray lower chest or collarbone'
      when v_mode = 'bold' then
        c.anchor_name || ': 1 spray chest, 1 spray back of neck | ' || c.layer_name || ': 1 spray opposite side of neck or collarbone'
      else
        c.anchor_name || ': 1 spray chest, 1 spray neck | ' || c.layer_name || ': 1 spray wrist or collarbone'
    end as placement_hint,

    case
      when v_mode = 'wild' then '1 spray anchor / 1 spray layer'
      else '2 sprays anchor / 1 spray layer'
    end as spray_guidance,

    case
      when v_mode = 'balance' then
        'Balanced mode — support-first pairing with ' || c.masking_risk_band_estimate || ' masking risk.'
      when v_mode = 'smooth' then
        'Smooth mode — softer support role that keeps the anchor in front.'
      when v_mode = 'bold' then
        'Bold mode — stronger contrast, still filtered to keep the layer supportive and the anchor in front.'
      when v_mode = 'wild' then
        'Wild mode — higher contrast with separate placement so the layer stays subordinate to the anchor.'
      else
        'Balanced mode — support-first pairing with ' || c.masking_risk_band_estimate || ' masking risk.'
    end as reason,

    c.anchor_name || ' stays the anchor, while ' || c.layer_name || ' behaves more like '
    || case
      when c.support_role_estimate = 'lift' then 'a lift/support accent'
      when c.support_role_estimate = 'lingering-base' then 'a lingering base support'
      else 'support rather than a true co-lead'
    end || '. '
    || c.opening_lead_name_estimate || ' should show more in the opening, while '
    || c.drydown_lead_name_estimate || ' should stay more present later.'
    || ' Overall intensity should stay anchored to '
    || c.anchor_name || '.'
    || ' Masking risk is '
    || c.masking_risk_band_estimate || '.'
    as why_it_works
  from chosen c;
