      ...
      select jsonb_build_object(
        ...
        'insights', jsonb_build_object(
          ...
          'signature_gravity', jsonb_build_object(
            ...
            'source', case
              when pr.preference_count > 0 or rr.wear_more_count > 0 then 'user_fragrance_preferences_v1 + user_fragrance_rotation_preferences_v1 + user_collection_effective_items_v2 + wear_events'
              when cs.bottle_count > 0 then 'user_collection_effective_items_v2 + wear_events'
              else null
            end,
            'empty_reason', case
              when pr.preference_count > 0 or rr.wear_more_count > 0 or cs.bottle_count > 0 then null
