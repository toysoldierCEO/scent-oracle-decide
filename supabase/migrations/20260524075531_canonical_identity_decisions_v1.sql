  ...
  loop
    ...
    if v_blocker_reason is null then
      ...
      if not found then
        ...
        v_blocker_reason := 'latest_review_status_not_needs_review';
      end if;
    end if;
...
comment on function public.decide_canonical_identity_reviews_v1(jsonb, text, boolean) is
  'Records explicit canonical identity review decisions only. It supersedes prior needs_review records and inserts decision records in fragrance_canonical_identity_reviews_v1 without merging rows, applying aliases, copying evidence, mutating public.fragrances, staging/promoting enrichment, writing taxonomy, refreshing performance, or touching frontend payloads.';

revoke all on function public.decide_canonical_identity_reviews_v1(jsonb, text, boolean)
  from public, anon, authenticated;

grant execute on function public.decide_canonical_identity_reviews_v1(jsonb, text, boolean)
  to service_role;
